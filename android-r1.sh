...
APK_URL="https://github.com/trunghieu1604/r1-ai/releases/download/v1.0/ai-box-plus.apk"
...
step_hide_packages() {
    ...
    local apps="player airskill exceptionreporter ijetty netctl systemtool device otaservice productiontest bugreport"
    for app in $apps; do
...
step_uninstall_existing() {
    ...
    local out
    out=$(adb_exec shell /system/bin/pm list packages | grep -F "$PACKAGE_NAME" || true)
